...
float getDist ( vec2 p){
  float circ = sdCircle(p - iResolution / 2.0 + vec2(0, -200), 100.0);
  float heart =sdHeart((p - iResolution / 2.0 + vec2(300, -150)) / 100.0) * 100.0;
  float blobCross = sdBlobbyCross((p - iResolution / 2.0 + vec2(-300, -200)) / 100.0 , 0.4) * 100.0;
  float moon = sdMoon(p - iResolution / 2.0 + vec2(0, 200), 50.0, 110.0, 65.0);
  float shoe = sdHorseshoe((p - iResolution / 2.0 + vec2(-300, 200)) / 100.0, vec2(cos(PI / 2.5), sin(PI / 2.5)), 0.7, 0.5, 0.2) * 100.0;
  float rhomb = sdRhombus((p - iResolution / 2.0 + vec2(300, 200)), vec2(100, 50));
  return min(min(min(min(min(circ, heart), blobCross), moon), shoe), rhomb);
}
